...
class Unet3plus(keras.Model):
    ...
    def __init__(self, kernel_regularizer=None, bias_regularizer=None, data_format='channels_last',
        ...
        self.conv1 = UnetConv2(filters=64, kernel_size=(3, 3), data_format=data_format, kernel_initializer="he_normal",
                               kernel_regularizer=kernel_regularizer, bias_regularizer=bias_regularizer, strides=(1, 1),
                               n=encoder_conv_n,
        ...
        # enc5
        self.conv5 = UnetConv2(filters=1024, kernel_size=(3, 3), data_format=data_format,
                               kernel_initializer="he_normal",
                               kernel_regularizer=kernel_regularizer, bias_regularizer=bias_regularizer, strides=(1, 1),
                               n=encoder_conv_n,
                               reg_lambda=reg_lambda,
        ...
        self.enc5_ds = Unet3plus_deep_supervision(upsample_size=2 ** 4, name="enc5_ds",
                                                  ...
                                                  bias_regularizer=bias_regularizer, reg_lambda=reg_lambda
                                                  )
        ...
        self.cgm_conv = keras.layers.Conv2D(filters=1, kernel_size=(1, 1), strides=(1, 1), padding='same',
                                            ...
                                            name=f"cgm_conv")
        self.cgm_pool = tfa.layers.AdaptiveMaxPooling2D(output_size=1, data_format=data_format,
                                                        name="cgm_adaptive_pool")
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            hloss = hloss1 * self.loss_weights[0] + hloss2 * self.loss_weights[1] + hloss3 * self.loss_weights[
                2] + hloss4 * self.loss_weights[3] + hloss5 * self.loss_weights[4]
        ...
        return {"loss": self.loss_tracker.result(), "hybrid_loss": self.hloss_tracker.result(),
                "hybrid_loss1": self.hloss1_tracker.result(), "hybrid_loss2": self.hloss2_tracker.result(),
                "hybrid_loss3": self.hloss3_tracker.result(),
                "hybrid_loss4": self.hloss4_tracker.result(), "hybrid_loss5": self.hloss5_tracker.result(),
                "binary_crossentropy": self.bloss_tracker.result(),
                "dice": self.dice_metric.result(),
                "dice1": self.dice1_metric.result(), "dice2": self.dice2_metric.result(),
                "dice3": self.dice3_metric.result(),
                "dice4": self.dice4_metric.result(), "dice5": self.dice5_metric.result(),
    ...
    def metrics(self):
        ...
        return [self.loss_tracker, self.hloss_tracker,
                self.hloss1_tracker, self.hloss2_tracker, self.hloss3_tracker, self.hloss4_tracker, self.hloss5_tracker,
                self.bloss_tracker,
                self.dice_metric,
                self.dice1_metric, self.dice2_metric, self.dice3_metric, self.dice4_metric, self.dice5_metric,
    ...
    def test_step(self, data):
        ...
        hloss4 = self.hybrid_loss(y[0], y_pred[3])
        hloss5 = self.hybrid_loss(y[0], y_pred[4])
        hloss = hloss1 * self.loss_weights[0] + hloss2 * self.loss_weights[1] + hloss3 * self.loss_weights[2] + hloss4 * \
                self.loss_weights[3] + hloss5 * self.loss_weights[4]
        bloss = self.bce_loss(y_true_cls, y_pred_cls)
        loss = hloss + bloss + sum(self.losses)
...
class UnetConv2(keras.layers.Layer):
    def __init__(self,
        ...
        for i in range(1, n + 1):
            self.ld['conv{}'.format(i)] = keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides,
                                                              padding="same",
                                                              kernel_regularizer=kernel_regularizer(reg_lambda),
                                                              bias_regularizer=bias_regularizer(reg_lambda),
                                                              data_format=data_format,
                                                              kernel_initializer=kernel_initializer,
                                                              name=f"{name}_unetconv2_conv{i}")
...
class FullScaleBlock(keras.layers.Layer):
    ...
    def __init__(self, num_smaller_scale,
        ...
        for i in range(1, num_smaller_scale + 1):
            self.ld['maxpool_{}'.format(i)] = keras.layers.MaxPool2D(pool_size=2 ** (num_smaller_scale - i + 1),
                                                                     data_format=data_format,
                                                                     name=f"{name}_fsb_maxpool_{i}")

        for i in range(1, 5 + 1):
            self.ld['conv_{}'.format(i)] = keras.layers.Conv2D(filters=filters, kernel_size=kernel_size,
        ...
        for i in range(num_smaller_scale + 2, 5 + 1):
            self.ld['bilinear_upsample_{}'.format(i)] = keras.layers.UpSampling2D(size=2 ** (i - self.nsc - 1),
                                                                                  data_format=data_format,
                                                                                  interpolation='bilinear',
                                                                                  name=f"{name}_fsb_bilinear_upsample_{i}")

        self.ld['concat'] = keras.layers.Concatenate(axis=-1, name=f"{name}_fsb_concat")

        self.ld['fusion_conv'] = keras.layers.Conv2D(filters=320, kernel_size=kernel_size, strides=strides,
                                                     padding='same',
                                                     kernel_regularizer=kernel_regularizer(reg_lambda),
                                                     bias_regularizer=bias_regularizer(reg_lambda),
                                                     data_format=data_format, kernel_initializer=kernel_initializer,
        ...
        self.ld['sd_biliear_upsample'] = keras.layers.UpSampling2D(size=2 ** (self.nsc), data_format=data_format,
                                                                   interpolation='bilinear',
                                                                   name=f"{name}_fsb_sd_bilinear_upsample")
    ...
    def call(self, inputs):
        ...
        inp_d['inp{}'.format(self.nsc + 1)] = self.ld['conv_{}'.format(self.nsc + 1)](
            inp_d['inp{}'.format(self.nsc + 1)])
        inp_d['inp{}'.format(self.nsc + 1)] = self.ld['bn_{}'.format(self.nsc + 1)](inp_d['inp{}'.format(self.nsc + 1)])
        inp_d['inp{}'.format(self.nsc + 1)] = self.ld['relu_{}'.format(self.nsc + 1)](
            inp_d['inp{}'.format(self.nsc + 1)])
